...
class GPWaterGeneralTaxService extends BaseService {
    ...
    async saveGpWaterGeneralTax(details) {
    ...
    async getTaxTypeGpMasterValue(gpId, taxationPeriod, type) {
        return this.execute(async () => {
            const query = {
                gpId: gpId,
            ...
            const taxType = result.map(item => {
                const taxEntry = item.tax.find(tax => tax.type.equals(new mongoose.Types.ObjectId(type)));
                return {
                    gpId: item.gpId,
...
module.exports = new GPWaterGeneralTaxService(
    GpWaterGeneralTaxModel,
    "GpWaterGeneralTax"
);
